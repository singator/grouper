...
groups. Each group will be allocated a topic $`t`$, from a pool of
topics $`1,\ldots,T`$. It is possible that each topic $`t`$ is repeated
$`R_t`$ times across the class. In total, there are $`N`$ students in
the class.
...
where $`w_1`$ and $`w_2`$ are weights. They indicate which half of the
...
dissimilarity between student $`i`$ and student $`j`$ should be included
