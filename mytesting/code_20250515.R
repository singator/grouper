...
model <- MIPModel() %>%
  ...
  add_variable(z[i,j,t,r], i=1:(N-1), j=(i+1):N, t=1:(B*T), r=1:R, type="binary") %>%
  ...
  set_objective(
    #' MAXIMISE DIVERSITY
    w1*sum_over(z[i,j,t,r]*d[i,j], i=1:(N-1), j=(i+1):N, t=1:(B*T), r=1:R)+
  ...
  add_constraint(z[i,j,t,r]<=sum_over(m[i,g]*x[g,t,r], g=1:G), i=1:(N-1), j=(i+1):N, t=1:(B*T), r=1:R) %>%
  add_constraint(z[i,j,t,r]<=sum_over(m[j,g]*x[g,t,r], g=1:G), i=1:(N-1), j=(i+1):N, t=1:(B*T), r=1:R) %>%
  add_constraint(z[i,j,t,r]>=sum_over(m[i,g]*x[g,t,r], g=1:G)+sum_over(m[j,g]*x[g,t,r], g=1:G)-1, i=1:(N-1), j=(i+1):N, t=1:(B*T), r=1:R) %>%
  ...
  add_constraint(sum_over(m[i,g]*x[g,t,r]*s[i], i=1:N, g=1:G)>=smin, t=1:(B*T), r=1:R) %>% 
  add_constraint(sum_over(m[i,g]*x[g,t,r]*s[i], i=1:N, g=1:G)<=smax, t=1:(B*T), r=1:R)
